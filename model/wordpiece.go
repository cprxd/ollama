...
func (wpm WordPiece) words(s string) iter.Seq[string] {
	return func(yield func(string) bool) {
		...
		for whitespace := range strings.FieldsFuncSeq(string(runes), unicode.IsSpace) {
			// split on but keep punctuation
			var start int
			for start < len(whitespace) {
				end := strings.IndexFunc(whitespace[start:], unicode.IsPunct)
				if end < 0 {
					end = len(whitespace) - start
				} else if end == 0 {
					end = 1
				}

				if !yield(whitespace[start : start+end]) {
					return
				}
